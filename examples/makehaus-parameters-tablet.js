...
const { ParameterType } = require('@makeproaudio/parameters-js');
const fs = require('fs');
const path = require('path');

const jsonPath = path.join(__dirname, 'layouts/makehaus-parameters-tablet.json');
...
const animateBindsForward = ({ widgetStacks, typesStacks, bindStacks }) => {
  ...
  const colors = ['#ff0000', '#00ff00', '#0000ff', '#00ffff', '#448877', '#76ff47', '#5678FF', '#004545'];
  ...
  bindStacks.forEach((s) => {
    ...
    const mappedIdx = s.parameter().id().split('-')[1] % 17;
...
MakeHaus.init(
  ...
  () => {
    ...
    const dynamicWidgetStackNames = [1, 2, 3, 4, 5, 6, 7, 8].map((id) => 'stack-' + id);
    const typeButtonStackNames = [9, 10, 11, 12, 13, 14, 15, 16].map((id) => 'stack-' + id);
    const bindButtonStacknames = [17, 18, 19, 20, 21, 22, 23, 24].map((id) => 'stack-' + id);
